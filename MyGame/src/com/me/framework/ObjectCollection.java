...
import com.me.framework.interfaces.IObjectCollection;
...
public abstract class ObjectCollection<T extends ICollectionItem> extends
	...
	public boolean removeObject(Object t) {
		Class<? extends Object> cls = t.getClass();
		for (int i = 0; i < objectCollection.size(); i++)
			if (objectCollection.get(i).getClass() == cls) {
				IDrawable iobj = (IDrawable) objectCollection.get(i);
				iobj.setDead(true);
				iobj.setEnabled(false);
				iobj.setVisible(false);
				objectCollection.remove(iobj);
				return true;
			}
		return false;
	}
